...
def test_title_slide_does_not_include_topics_in_markdown(tmp_path):
    db_path = tmp_path / "test_presentations_topics.db"
    ...
    c.execute('INSERT INTO decks (id, presentation_id, week, date, topic1, topic2, order_index) VALUES (?, ?, ?, ?, ?, ?, ?)',
              (1, 1, 'Week 1', '2026-01-13', 'Topic Alpha', 'Topic Beta', 0))
    ...
    # Ensure topics are not rendered on slides
    assert 'Topic Alpha' not in content
    assert 'Topic Beta' not in content
