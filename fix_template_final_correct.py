#!/usr/bin/env python3
"""
Final fix for Gold layouts: Remove backgrounds but preserve list style fonts properly
"""
...
def fix_gold_layouts_properly():
        ...
        for i in range(1, 15):
            ...
            if os.path.exists(layout_path):
                ...
                if 'Gold_' in content:
                    ...
                    try:
                        # 1. Remove hardcoded background elements
                        content = re.sub(r'<p:bg>.*?</p:bg>', '', content, flags=re.DOTALL)
                        
                        # 2. Only remove problematic panose attributes, keep Helvetica Neue fonts
                        content = re.sub(r' panose="[^"]*"', '', content)
                        content = re.sub(r' pitchFamily="[^"]*"', '', content)
                        content = re.sub(r' charset="[^"]*"', '', content)
                        
                        # 3. DON'T remove lstStyle content - that's needed for body text!
                        # The lstStyle contains the font definitions that body text inherits from
    ...
    print(f"\nCorrect fixed template saved as: {fixed_template_path}")
    print(f"Gold layouts processed: {fixed_layouts}")
    return fixed_template_path

if __name__ == "__main__":
    fix_gold_layouts_properly()